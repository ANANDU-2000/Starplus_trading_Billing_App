...
const ProductsPage = () => {
  ...
  const handleCreateProduct = async (productData) => {
    ...
    try {
      ...
      // Handle 409 Conflict (duplicate SKU or concurrency issue)
      if (error?.response?.status === 409) {
        const errorMsg = error?.response?.data?.message || 'Product with this SKU already exists or was modified by another user'
  ...
  const handleStockAdjustmentSubmit = async (adjustmentData) => {
    try {
      ...
      const response = await productsAPI.adjustStock(selectedProduct.id, adjustmentData)
      if (response?.success) {
  ...
  return (
    <div className="space-y-3 sm:space-y-4 lg:space-y-6">
      ...
      <div className="bg-white border-b border-gray-200 shadow-sm -mx-2 sm:-mx-4 lg:-mx-6 px-2 sm:px-4 lg:px-6 py-2 sm:py-3 lg:py-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900">Products</h1>
              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                {totalCount} Total
              </span>
            </div>
            <p className="text-xs sm:text-sm text-gray-600 mt-0.5 sm:mt-1">Manage your inventory</p>
          </div>
          <div className="flex flex-wrap sm:flex-nowrap gap-2 sm:space-x-3 w-full sm:w-auto">
            <button
              onClick={() => loadProducts()}
              className="inline-flex items-center justify-center px-2 sm:px-3 lg:px-4 py-1.5 sm:py-2 border border-gray-300 rounded-lg shadow-sm text-xs sm:text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors flex-1 sm:flex-none"
            >
            ...
            <button
              ...
              className="inline-flex items-center justify-center px-2 sm:px-3 lg:px-4 py-1.5 sm:py-2 border border-transparent rounded-lg shadow-sm text-xs sm:text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 transition-colors flex-1 sm:flex-none"
            >
      ...
      <ModernTable
        ...
        columns={[
          ...
          { 
            ...
            sortable: true,
            render: (product) => {
              if (!product.expiryDate) return <span className="text-gray-400 text-xs">No expiry</span>;
              const expiryDate = new Date(product.expiryDate);
              const today = new Date();
              const daysUntilExpiry = Math.ceil((expiryDate - today) / (1000 * 60 * 60 * 24));
              
              if (daysUntilExpiry < 0) {
                return <span className="text-red-600 font-medium text-xs">⚠️ Expired</span>;
              } else if (daysUntilExpiry <= 30) {
                return <span className="text-orange-600 font-medium text-xs">⚠️ {daysUntilExpiry}d left</span>;
              } else {
