...
const ReportsPage = () => {
  ...
  const fetchReportData = useCallback(async (force = false) => {
    ...
    if (isTabChangingRef.current && !force) {
      console.log('⏸️ Tab change in progress, skipping fetch...')
      return
    }
    ...
    if (!force && lastRequestParamsRef.current === requestSignature) {
      console.log('⏸️ Duplicate request prevented (same params)')
      return
    }
    ...
    if (!force && timeSinceLastFetch < FETCH_THROTTLE_MS) {
      console.log(`⏸️ Throttling request (${timeSinceLastFetch}ms < ${FETCH_THROTTLE_MS}ms)`)
    ...
    try {
      ...
      if (error?.response?.status === 429) {
        console.log('⏸️ Rate limit exceeded, request throttled')
  ...
  // Sync with URL params on mount (only once, not on every searchParams change)
  useEffect(() => {
    const tabFromUrl = searchParams.get('tab')
    const pathName = window.location.pathname
    
    // Handle /reports/outstanding route
    if (pathName.includes('/outstanding')) {
      if (activeTab !== 'outstanding') {
        setActiveTab('outstanding')
        setSearchParams({ tab: 'outstanding' })
      }
    ...
    <div className="space-y-3 sm:space-y-4 lg:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-600">Comprehensive business insights and analytics</p>
        </div>
        <div className="mt-2 sm:mt-0 flex flex-wrap gap-2 sm:space-x-3">
          <button
            onClick={() => fetchReportData()}
            className="inline-flex items-center justify-center px-2 sm:px-3 lg:px-4 py-1.5 sm:py-2 border border-gray-300 rounded-md shadow-sm text-xs sm:text-sm font-medium text-gray-700 bg-white hover:bg-gray-50"
          >
          ...
          <button
            ...
            className="inline-flex items-center justify-center px-2 sm:px-3 lg:px-4 py-1.5 sm:py-2 border border-transparent rounded-md shadow-sm text-xs sm:text-sm font-medium text-white bg-blue-600 hover:bg-blue-700"
          >
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-4 lg:p-6">
        <div className="flex items-center mb-3 sm:mb-4">
          ...
          <h3 className="text-sm sm:text-base lg:text-lg font-semibold text-gray-900">Filters</h3>
        </div>
          ...
          <nav className="-mb-px flex space-x-2 sm:space-x-4 lg:space-x-8 px-2 sm:px-4 lg:px-6 overflow-x-auto">
            {tabs.map((tab) => {
              ...
              return (
                  ...
                  className={`py-2 sm:py-3 lg:py-4 px-1 sm:px-2 border-b-2 font-medium text-xs sm:text-sm flex items-center whitespace-nowrap ${
                    activeTab === tab.id
                      ...
                      <div key={customer.id || index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                        ...
                        <div className="text-right">
                          <p className={`font-semibold text-lg ${
                            customer.total > (customer.creditLimit * 0.8) 
                              ? 'text-red-600' 
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className={`text-4xl font-bold mt-2 ${
                          (reportData.profitLoss.netProfit || 0) >= 0 ? 'text-green-700' : 'text-red-700'
                        }`}>
                      ...
                      <TrendingUp className={`h-16 w-16 ${
                        (reportData.profitLoss.netProfit || 0) >= 0 ? 'text-green-600' : 'text-red-600'
                      }`} />
                    </div>
                  </div>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                              {bill.daysOverdue > 0 ? (
                                <span className={`font-medium ${
                                  bill.daysOverdue > 90 ? 'text-red-600' :
