...
namespace FrozenApi.Services
{
    ...
    public class ComprehensiveBackupService : IComprehensiveBackupService
    {
        ...
        public ComprehensiveBackupService(AppDbContext context, IConfiguration configuration, IServiceProvider serviceProvider)
        {
            ...
            _desktopPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "StarPlus_Backups");
        ...
        public async Task<string> CreateFullBackupAsync(bool exportToDesktop = false, bool uploadToGoogleDrive = false, bool sendEmail = false)
        {
            ...
            var zipFileName = $"StarPlus_Backup_{timestamp}.zip";
        ...
        private async Task BackupDatabaseAsync(ZipArchive zipArchive, string timestamp)
        {
            ...
            if (!File.Exists(fullDbPath))
            {
                ...
                throw new FileNotFoundException($"SQLite database file not found at: {fullDbPath}");
            }
        ...
        private async Task BackupMonthlySalesLedgerAsync(ZipArchive zipArchive)
        {
            ...
            try
            {
                ...
                var currentMonthLedger = await reportService.GetComprehensiveSalesLedgerAsync(currentMonthStart, currentMonthEnd);
                var currentMonthPdf = await pdfService.GenerateSalesLedgerPdfAsync(currentMonthLedger, currentMonthStart, currentMonthEnd);
                ...
                var previousMonthLedger = await reportService.GetComprehensiveSalesLedgerAsync(previousMonthStart, previousMonthEnd);
                var previousMonthPdf = await pdfService.GenerateSalesLedgerPdfAsync(previousMonthLedger, previousMonthStart, previousMonthEnd);
        ...
        private async Task SendBackupEmailAsync(string zipPath)
        {
            try
            {
                ...
                var adminEmail = settings.GetValueOrDefault("ADMIN_EMAIL", 
                    _configuration["BackupSettings:Email:ToEmail"] ?? "admin@starplus.com");
