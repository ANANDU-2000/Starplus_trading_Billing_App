    ...
    public class ProductSeedService : IProductSeedService
    {
        ...
        public async Task SeedProductsFromExcelAsync()
        {
            ...
            try
            {
                ...
                // Parent of current directory (project root if running from backend/FrozenApi)
                ...
                foreach (var excelFile in excelFiles)
                {
                    ...
                    {
                        _logger.LogError(ex, $"Failed to import products from {excelFile}");
                    }
                }

                var finalCount = await context.Products.CountAsync();
                _logger.LogInformation(
                    $"Product seeding completed. Total products in database: {finalCount} " +
                    $"({totalImported} imported, {totalUpdated} updated from Excel files)");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error during product seeding from Excel");
