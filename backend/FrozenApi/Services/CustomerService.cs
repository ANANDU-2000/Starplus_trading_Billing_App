...
namespace FrozenApi.Services
{
    ...
    {
        Task<PagedResponse<CustomerDto>> GetCustomersAsync(int page = 1, int pageSize = 10, string? search = null);
        ...
        Task<(bool Success, string Message, DeleteCustomerSummary? Summary)> ForceDeleteCustomerWithAllDataAsync(int customerId, int userId);
        Task<List<CustomerLedgerEntry>> GetCustomerLedgerAsync(int customerId);
        Task<List<CustomerLedgerEntry>> GetCashCustomerLedgerAsync();
        Task<byte[]> GenerateCustomerStatementAsync(int customerId, DateTime fromDate, DateTime toDate);
        Task<List<CustomerDto>> SearchCustomersAsync(string query, int limit = 20);
    ...
    public class CustomerService : ICustomerService
    {
        ...
        public async Task<PagedResponse<CustomerDto>> GetCustomersAsync(int page = 1, int pageSize = 10, string? search = null)
        {
        ...
        public async Task<(bool Success, string Message)> DeleteCustomerAsync(int id)
        {
            ...
            if (hasSales || hasPayments)
            {
                ...
                var salesCount = await _context.Sales.CountAsync(s => s.CustomerId == id && !s.IsDeleted);
                var paymentsCount = await _context.Payments.CountAsync(p => p.CustomerId == id);
        ...
        /// </summary>
        public async Task<(bool Success, string Message, DeleteCustomerSummary? Summary)> ForceDeleteCustomerWithAllDataAsync(int customerId, int userId)
        {
            using var transaction = await _context.Database.BeginTransactionAsync();
            try
            {
        ...
        /// Rules: One row per transaction, clear invoice-payment links, running balance
        /// </summary>
        public async Task<List<CustomerLedgerEntry>> GetCustomerLedgerAsync(int customerId)
        {
            // CRITICAL: Ensure customerId is valid and filter out any null CustomerId records
            if (customerId <= 0)
                throw new ArgumentException("Invalid customer ID");

            // Get ALL sales (non-deleted only) - STRICT filtering by customerId, exclude null CustomerId
            var sales = await _context.Sales
                .Where(s => s.CustomerId.HasValue && s.CustomerId.Value == customerId && !s.IsDeleted)
                .OrderBy(s => s.InvoiceDate)
                .ThenBy(s => s.Id)
                .ToListAsync();

            // Get ALL payments - STRICT filtering by customerId, exclude null CustomerId
            var payments = await _context.Payments
                .Where(p => p.CustomerId.HasValue && p.CustomerId.Value == customerId)
                .OrderBy(p => p.PaymentDate)
                .ThenBy(p => p.Id)
                .ToListAsync();

            // Get sales returns - STRICT filtering by customerId, exclude null CustomerId
            var salesReturns = await _context.SaleReturns
                .Where(sr => sr.CustomerId.HasValue && sr.CustomerId.Value == customerId)
            ...
            var salePayments = await _context.Payments
                .Where(p => p.CustomerId.HasValue && p.CustomerId.Value == customerId && p.SaleId.HasValue)
            ...
            foreach (var transaction in groupedTransactions)
            {
                ...
                var transactionKey = (transaction.SaleId, transaction.PaymentId, transaction.Date.Date, transaction.Debit + transaction.Credit);
        ...
        /// </summary>
        public async Task<List<CustomerLedgerEntry>> GetCashCustomerLedgerAsync()
        {
            // Get ALL cash customer sales (CustomerId is null)
            var sales = await _context.Sales
                .Where(s => s.CustomerId == null && !s.IsDeleted)
                .OrderBy(s => s.InvoiceDate)
                .ThenBy(s => s.Id)
                .ToListAsync();

            // Get ALL cash customer payments (CustomerId is null)
            var payments = await _context.Payments
            ...
            // Get sales returns for cash customers (CustomerId is null)
            var salesReturns = await _context.SaleReturns
                .Where(sr => sr.CustomerId == null)
                .OrderBy(sr => sr.ReturnDate)
                .ToListAsync();
            ...
            var salePayments = await _context.Payments
                .Where(p => p.CustomerId == null && p.SaleId.HasValue)
                .GroupBy(p => p.SaleId!.Value)
                .Select(g => new { SaleId = g.Key, TotalPaid = g.Sum(p => p.Amount) })
                .ToDictionaryAsync(x => x.SaleId, x => x.TotalPaid);
        ...
        public async Task<byte[]> GenerateCustomerStatementAsync(int customerId, DateTime fromDate, DateTime toDate)
        {
            try
            {
            ...
            var companyName = settings.GetValueOrDefault("COMPANY_NAME_EN") ?? "STARPLUS FOODSTUFF TRADING";
            ...
            var currency = settings.GetValueOrDefault("CURRENCY") ?? "AED";
                ...
                if (string.IsNullOrEmpty(companyName))
                    companyName = "STARPLUS FOODSTUFF TRADING";
            ...
            var openingSales = await _context.Sales
                .Where(s => s.CustomerId.HasValue && s.CustomerId.Value == customerId && !s.IsDeleted && s.InvoiceDate < fromDate)
            ...
            var openingPayments = await _context.Payments
                .Where(p => p.CustomerId.HasValue && p.CustomerId.Value == customerId && p.PaymentDate < fromDate)
                .SumAsync(p => (decimal?)p.Amount) ?? 0;
            
            // Include sales returns (credit) in opening balance
            var openingSalesReturns = await _context.SaleReturns
                .Where(sr => sr.CustomerId.HasValue && sr.CustomerId.Value == customerId && sr.ReturnDate < fromDate)
        ...
        public async Task<List<CustomerDto>> SearchCustomersAsync(string query, int limit = 20)
        {
            // NOTE: DatabaseFixer should only run at application startup (Program.cs)
            // Columns should exist from migrations.
        ...
        public async Task RecalculateCustomerBalanceAsync(int customerId)
        {
            ...
            // Calculate total payments (credits) - use cleared payments only (Status = CLEARED)
            ...
            // Calculate actual balance: Sales - Payments - SalesReturns = Outstanding Balance
            var actualBalance = totalSales - totalPayments - totalSalesReturns;

            customer.Balance = actualBalance;
            customer.UpdatedAt = DateTime.UtcNow;

            await _context.SaveChangesAsync();
            Console.WriteLine($"✅ Recalculated balance for {customer.Name}: {actualBalance:C} (Sales: {totalSales:C}, Payments: {totalPayments:C}, Returns: {totalSalesReturns:C})");
        }
        ...
        public async Task RecalculateAllCustomerBalancesAsync()
        {
            ...
            foreach (var customer in customers)
            {
                ...
                var totalPayments = await _context.Payments
                    .Where(p => p.CustomerId == customer.Id && p.Status == PaymentStatus.CLEARED)
                    .SumAsync(p => (decimal?)p.Amount) ?? 0;
                ...
                customer.Balance = totalSales - totalPayments - totalSalesReturns;
                customer.UpdatedAt = DateTime.UtcNow;
            }

            await _context.SaveChangesAsync();
            Console.WriteLine($"✅ Recalculated balances for {customers.Count} customers");
        }
