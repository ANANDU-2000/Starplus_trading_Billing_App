...
namespace FrozenApi.Services
{
    ...
    {
        Task<SupplierBalanceDto> GetSupplierBalanceAsync(string supplierName);
        Task<List<SupplierTransactionDto>> GetSupplierTransactionsAsync(string supplierName, DateTime? fromDate = null, DateTime? toDate = null);
        Task<List<SupplierSummaryDto>> GetAllSuppliersSummaryAsync();
    }
    ...
    public class SupplierService : ISupplierService
    {
        ...
        public async Task<SupplierBalanceDto> GetSupplierBalanceAsync(string supplierName)
        {
            // Calculate total purchases (what we owe)
            var totalPurchases = await _context.Purchases
                .Where(p => p.SupplierName == supplierName)
                .SumAsync(p => (decimal?)p.TotalAmount) ?? 0;

            // Calculate total purchase returns (credit)
            var totalPurchaseReturns = await _context.PurchaseReturns
                .Include(pr => pr.Purchase)
                .Where(pr => pr.Purchase.SupplierName == supplierName)
        ...
        public async Task<List<SupplierTransactionDto>> GetSupplierTransactionsAsync(string supplierName, DateTime? fromDate = null, DateTime? toDate = null)
        {
        ...
        public async Task<List<SupplierSummaryDto>> GetAllSuppliersSummaryAsync()
        {
            ...
            {
                var balance = await GetSupplierBalanceAsync(supplierName);
