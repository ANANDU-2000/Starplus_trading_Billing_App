...
namespace FrozenApi.Services
{
    ...
    {
        Task<SaleReturnDto> CreateSaleReturnAsync(CreateSaleReturnRequest request, int userId);
        Task<PurchaseReturnDto> CreatePurchaseReturnAsync(CreatePurchaseReturnRequest request, int userId);
        Task<List<SaleReturnDto>> GetSaleReturnsAsync(int? saleId = null);
        Task<List<PurchaseReturnDto>> GetPurchaseReturnsAsync(int? purchaseId = null);
    }
    ...
    public class ReturnService : IReturnService
    {
        ...
        public async Task<SaleReturnDto> CreateSaleReturnAsync(CreateSaleReturnRequest request, int userId)
        {
            ...
            try
            {
                ...
                var returnNo = await GenerateSaleReturnNumberAsync();
                ...
                {
                    await _customerService.RecalculateCustomerBalanceAsync(sale.CustomerId.Value);
                }
        ...
        public async Task<PurchaseReturnDto> CreatePurchaseReturnAsync(CreatePurchaseReturnRequest request, int userId)
        {
            using var transaction = await _context.Database.BeginTransactionAsync();
            try
            {
                ...
                // Generate return number
                var returnNo = await GeneratePurchaseReturnNumberAsync();
        ...
        public async Task<List<SaleReturnDto>> GetSaleReturnsAsync(int? saleId = null)
        {
        ...
        public async Task<List<PurchaseReturnDto>> GetPurchaseReturnsAsync(int? purchaseId = null)
        {
        ...
        private async Task<string> GenerateSaleReturnNumberAsync()
        {
            var today = DateTime.Today.ToString("yyyyMMdd");
            var lastReturn = await _context.SaleReturns
                .Where(r => r.ReturnNo.StartsWith($"RET-{today}"))
        ...
        private async Task<string> GeneratePurchaseReturnNumberAsync()
        {
            ...
            var lastReturn = await _context.PurchaseReturns
                .Where(r => r.ReturnNo.StartsWith($"PUR-RET-{today}"))
                .OrderByDescending(r => r.ReturnNo)
                .FirstOrDefaultAsync();
