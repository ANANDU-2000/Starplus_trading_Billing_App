    ...
    public static class DatabaseFixer
    {
        public static async Task FixMissingColumnsAsync(DbContext context)
        {
            ...
            try
            {
                ...
                // So we'll try each command and ignore errors if column already exists
                var commands = new[]
                {
                ...
                foreach (var (command, tableName, columnName) in commands)
                {
                    ...
                    {
                        // Column might already exist, ignore (this is expected and normal)
                        if (ex.Message.Contains("duplicate column") || 
                            ex.Message.Contains("already exists") ||
                            ex.Message.Contains("duplicate column name") ||
                            ex.Message.Contains("SQLite Error 1") ||
                            ex.Message.Contains("SQL logic error") ||
                            ex.InnerException?.Message.Contains("duplicate") == true)
                        {
                            columnsSkipped++;
                            // Column exists, that's fine - completely silent (no log)
                        }
                        else
                        {
                            // Only log unexpected errors
                            logger?.LogWarning("Unexpected error adding {Table}.{Column}: {Message}", tableName, columnName, ex.Message);
                        }
                    }
                }
                ...
                // ExecuteSqlRawAsync handles connection management automatically, no need to close manually
