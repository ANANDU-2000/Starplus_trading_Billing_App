    ...
    public static class SecurityConfiguration
    {
        public static IServiceCollection AddSecurityServices(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            if (!string.IsNullOrEmpty(allowedOriginsEnv))
            {
                ...
                // Always ensure Netlify origin is included
                var netlifyOrigin = "https://starplusposbilingsystem.netlify.app";
                if (!envOrigins.Contains(netlifyOrigin))
                {
                    envOrigins.Add(netlifyOrigin);
                    Console.WriteLine($"CORS: Auto-added Netlify origin: {netlifyOrigin}");
                }
            ...
            {
                // Fallback to appsettings.json configuration with default Netlify URLs
                allowedOrigins = configuration.GetSection("AllowedOrigins").Get<string[]>() ?? 
                                new[] { 
                                    "http://localhost:3000", 
                                    "https://localhost:3000",
                                    "https://starplusposbilingsystem.netlify.app"
                                };
