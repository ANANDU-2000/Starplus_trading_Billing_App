    ...
    public class AdminController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse<string>>> UploadLogo([FromForm] IFormFile file)
        {
            try
            {
                ...
                // Also update COMPANY_LOGO setting
                var logoSetting = await _context.Settings
                    .FirstOrDefaultAsync(s => s.Key.ToUpper() == "COMPANY_LOGO" || s.Key.ToUpper() == "COMPANY_LOGO_URL");
        ...
        public async Task<ActionResult> DownloadBackup(string fileName)
        {
            try
            {
                ...
                if (!System.IO.File.Exists(backupPath))
                {
                    ...
                    var desktopPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "StarPlus_Backups", fileName);
