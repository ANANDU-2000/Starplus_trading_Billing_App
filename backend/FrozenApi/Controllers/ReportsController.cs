/*
Purpose: Reports controller for analytics and AI suggestions
Author: AI Assistant
Date: 2024
*/
...
namespace FrozenApi.Controllers
{
    ...
    public class ReportsController : ControllerBase
    {
            ...
            [FromQuery] DateTime? toDate = null)
        {
            try
            {
                Console.WriteLine($"📊 GetSummaryReport endpoint called with fromDate: {fromDate}, toDate: {toDate}");
                var result = await _reportService.GetSummaryReportAsync(fromDate, toDate);
        ...
        public async Task<ActionResult<ApiResponse<PagedResponse<SaleDto>>>> GetSalesReport(
            ...
            [FromQuery] string? status = null, // Pending, Paid, Partial
            ...
            [FromQuery] int days = 30)
        {
            try
            {
                var result = await _reportService.GetOutstandingCustomersAsync(days);
        ...
        public async Task<ActionResult<ApiResponse<List<PaymentDto>>>> GetChequeReport()
        {
            try
            {
                var result = await _reportService.GetChequeReportAsync();
            ...
            [FromQuery] int periodDays = 30)
        {
            try
            {
                var result = await _reportService.GetAISuggestionsAsync(periodDays);
            ...
            [FromQuery] DateTime? toDate = null)
        {
            try
            {
                ...
                var result = await _reportService.GetExpensesByCategoryAsync(from, to);
            ...
            [FromQuery] string groupBy = "day")
        {
            try
            {
                var from = fromDate ?? DateTime.Today.AddDays(-30);
                var to = toDate ?? DateTime.Today;
                var result = await _reportService.GetSalesVsExpensesAsync(from, to, groupBy);
            ...
            [FromQuery] DateTime? toDate = null)
        {
            try
            {
                ...
                // Get sales data
                var salesData = await _reportService.GetSalesReportAsync(from, to, null, null, 1, 1000);
            ...
            [FromQuery] int pageSize = 50)
        {
            try
            {
                var from = fromDate ?? DateTime.Today.AddDays(-30);
                var to = toDate ?? DateTime.Today;
                var result = await _reportService.GetEnhancedSalesReportAsync(from, to, granularity, productId, customerId, status, page, pageSize);
            ...
            [FromQuery] bool lowStockOnly = false)
        {
            try
            {
                var from = fromDate ?? DateTime.Today.AddDays(-30);
                var to = toDate ?? DateTime.Today;
                var result = await _reportService.GetEnhancedProductSalesReportAsync(from, to, productId, unitType, lowStockOnly);
                return Ok(new ApiResponse<List<ProductSalesDto>>
                {
                    Success = true,
            ...
            [FromQuery] decimal? minOutstanding = null)
        {
            try
            {
                var from = fromDate ?? DateTime.Today.AddDays(-30);
                var to = toDate ?? DateTime.Today;
                var result = await _reportService.GetCustomerReportAsync(from, to, minOutstanding);
            ...
            [FromQuery] int? customerId = null)
        {
            try
            {
                ...
                var result = await _reportService.GetAgingReportAsync(asOf, customerId);
            ...
            [FromQuery] bool lowOnly = false)
        {
            try
            {
                var result = await _reportService.GetStockReportAsync(lowOnly);
            ...
            [FromQuery] DateTime? toDate = null)
        {
            try
            {
                var result = await _reportService.GetComprehensiveSalesLedgerAsync(fromDate, toDate);
            ...
            [FromQuery] string? type = null)
        {
            try
            {
                ...
                var ledgerReport = await _reportService.GetComprehensiveSalesLedgerAsync(from, to);
            ...
            [FromQuery] DateTime? toDate = null)
        {
            try
            {
                ...
                var salesData = await _reportService.GetSalesReportAsync(from, to, null, null, 1, 10000);
        ...
        private async Task<string> GenerateSalesReportHtmlAsync(PagedResponse<SaleDto> salesData, InvoiceTemplateService.CompanySettings settings, DateTime from, DateTime to)
        {
            ...
            return template
                ...
                .Replace("{{generated_date}}", DateTime.Now.ToString("dd-MM-yyyy HH:mm"))
